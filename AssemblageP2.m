function [K,F] = AssemblageP2(alpha,beta,gamma,f, X, T)
...
%
   for k=1:t   % boucle sur les éléments
       elemKi = mat_elem_P2(alpha,beta,gamma,X,T,k);  % matrice de l'élément numéro k
...
%
   for k=1:t   % boucle sur les éléments
       elemFi = SM_elem_P2(f,X,T,k);  % SM de l'élément numéro k
     for i=1:3  %boucle sur les numéros locaux
     I=2*k+i-2;                            % numéros globaux dans K
